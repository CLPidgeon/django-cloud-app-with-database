...
def show_exam_result(request, course_id, submission_id, lesson_id):
    context = {}
    ...
    total = 0
    for i in submit.choices.all().values_list('question_id'):
        total += Question.objects.filter(id=i[0]).first().grade 
    grade_percentage = 0
    if score > 0:
        grade_percentage = int((score//total) * 100)
    context['selected'] = selected
    context['grade'] = score
    context['course'] = course
    context['lesson'] = lesson
    context['total'] = total
    context['grade_percentage'] = grade_percentage
